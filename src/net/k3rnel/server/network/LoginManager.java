...
import net.k3rnel.server.backend.entity.MonsterBox;
...
public class LoginManager implements Runnable {
	...
	private void attemptLogin(IoSession session, char l, String username, String password) {
		try {
			...
			ResultSet result = m_database.query("SELECT * FROM kn_bans WHERE ip='" + getIp(session) + "'");
			...
			result = m_database.query("SELECT * FROM kn_members WHERE username='" + MySqlManager.parseSQL(username) + "'");
			if(!result.first()){
			...
			if(result.getString("password").compareTo(password) == 0) {
				...
				if(result.getString("lastLoginServer").equalsIgnoreCase(GameServer.getServerName())) {
					...
					if(TcpProtocolHandler.containsPlayer(username)) {
						...
						m_database.query("UPDATE kn_members SET lastLoginServer='" + MySqlManager.parseSQL(GameServer.getServerName()) + "', lastLoginTime='" + time + "' WHERE username='" + MySqlManager.parseSQL(username) + "'");
						m_database.query("UPDATE kn_members SET lastLoginIP='" + getIp(session) + "' WHERE username='" + MySqlManager.parseSQL(username) + "'");
						m_database.query("UPDATE kn_members SET lastLanguageUsed='" + l + "' WHERE username='" + MySqlManager.parseSQL(username) + "'");
...
//					}
			 ...
			 */
			m_database.query("UPDATE kn_members SET lastLoginServer='null' WHERE username='" + MySqlManager.parseSQL(username) + "'");
			m_database.close();
		}
	}
	...
	private void changePass(String username, String newPassword, String oldPassword, IoSession session) {
		...
		if(m_database.connect(GameServer.getDatabaseHost(), GameServer.getDatabaseUsername(), GameServer.getDatabasePassword())) {
			if(m_database.selectDatabase(GameServer.getDatabaseName())) {
				ResultSet result = m_database.query("SELECT * FROM kn_members WHERE username='" + MySqlManager.parseSQL(username) + "'");
				try {
					if(result.first()){
						...
						if(result.getString("password").compareTo(oldPassword) == 0) {
							...
							m_database.query("UPDATE kn_members SET password='" + MySqlManager.parseSQL(newPassword) + "' WHERE username='" + MySqlManager.parseSQL(username) + "'");
	...
	private void login(String username, char language, IoSession session, ResultSet result) {
		...
		m_database.query("UPDATE kn_members SET lastLoginServer='" + MySqlManager.parseSQL(GameServer.getServerName()) + "', lastLoginTime='" + time + "' WHERE username='" + MySqlManager.parseSQL(username) + "'");
		m_database.query("UPDATE kn_members SET lastLoginIP='" + getIp(session) + "' WHERE username='" + MySqlManager.parseSQL(username) + "'");
		m_database.query("UPDATE kn_members SET lastLanguageUsed='" + language + "' WHERE username='" + MySqlManager.parseSQL(username) + "'");
		session.setAttribute("player", p);
		/*
...
//		p.updateClientFriends();
	...
	private PlayerChar getPlayerObject(ResultSet result) {
		try {
			...
			MonsterBox[] boxes = new MonsterBox[9];
			...
			//Retrieve refences to all Monsters
			int partyId = result.getInt("party");
			ResultSet partyData = m_database.query("SELECT * FROM kn_party WHERE id='" + partyId + "'");
			partyData.first();
			
			ResultSet monsters = m_database.query("SELECT * FROM kn_pokemon WHERE currentTrainerName='" + p.getName() + "'");
			int boxNumber = 0;
			int boxPosition = 0;
			/* Loop through all Monsters belonging to this player and add them to their party/box */
			while(monsters.next()) {
				boolean isParty = false;
				int partyIndex = -1;
				/* Checks if Monster is in party */
				for(int i = 0; i < 6; i++) {
					if(partyData.getInt("monster" + i) == monsters.getInt("id")) {
						isParty = true;
						partyIndex = i;
						break;
					}
				}
				/* If the monster is in party, add it to party */
				if(isParty) {
					party[partyIndex] = getMonsterObject(monsters);
				} else {
					/* Else, add it to box if space is available */
					if(boxNumber < 9) {
						/* Avoid null pointers */
						if(boxes[boxNumber] == null)
							boxes[boxNumber] = new MonsterBox();
						if(boxes[boxNumber].getMonsters() == null)
							boxes[boxNumber].setMonsters(new Monster[30]);
						/* If there's space in this box, add it to the box */
						if(boxPosition < 30) {
							boxes[boxNumber].setMonster(boxPosition, getMonsterObject(monsters));
						} else {
			...
			//Attach bag
			p.setBag(getBagObject(m_database.query("SELECT * FROM kn_bag WHERE member='" + result.getInt("id") + "'"),p.getId()));
	...
	/**
	 * Returns a Monster object based on a set of data
	 * @param data
	 * @return
	 */
	private Monster getMonsterObject(ResultSet data) {
		if(data != null) {
			try {
				/*
				...
				Monster p = new Monster(
						...
						MonsterSpecies.getDefaultData().getMonsterByName(data.getString("speciesName"))
						,
						MonsterNature.getNatureByName(data.getString("nature")),
