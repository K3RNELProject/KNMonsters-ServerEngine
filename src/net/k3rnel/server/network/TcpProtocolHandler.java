...
import net.k3rnel.server.backend.ItemProcessor;
...
import net.k3rnel.server.network.message.RequestMessage;
...
public class TcpProtocolHandler extends IoHandlerAdapter {
		...
		if(session.getAttribute("player") == null) {
			...
			case 'P':
				...
				case 'm':
					...
					monsterIndex = Integer.parseInt(String.valueOf(message.charAt(2)));
					int moveIndex = Integer.parseInt(String.valueOf(message.charAt(3)));
					move = message.substring(4);
					if(move != null && !move.equalsIgnoreCase("") &&
							p.getParty()[monsterIndex] != null) {
						if(p.getParty()[monsterIndex].getMovesLearning().contains(move)) {
							p.getParty()[monsterIndex].learnMove(moveIndex, move);
							p.updateClientPP(monsterIndex, moveIndex);
						}
					}
					break;
				case 'M':
					...
					if(p.getParty()[monsterIndex] != null) {
						if(p.getParty()[monsterIndex].getMovesLearning().contains(move)) {
							p.getParty()[monsterIndex].getMovesLearning().remove(move);
						}
					}
					break;
				case 'e':
					//Evolution response
					monsterIndex = Integer.parseInt(String.valueOf(message.charAt(3)));
					if(p.getParty()[monsterIndex] != null) {
						switch(message.charAt(2)) {
						case '0':
							//Cancel evolution
							p.getParty()[monsterIndex].evolutionResponse(false, p);
							break;
						case '1':
							//Allow evolution
							p.getParty()[monsterIndex].evolutionResponse(true, p);
							break;
						}
					}
					break;
			...
			case 's':
				//Party swapping
				p.swapMonsters(Integer.parseInt(message.substring(1, message.indexOf(','))), 
			...
			case 'B':
				...
				if(p.isBoxing()) {
					...
					case 'r':
						...
						break;
					case 'R':
						//Releasing a monster from storage - sent as BRBOXNUM,BOXSLOT
						details = message.substring(2).split(",");
						p.releaseMonster(Integer.parseInt(details[0]), Integer.parseInt(details[1]));
						break;
					case 's':
						//Swap monster between box and party - sent as BsBOXNUM,BOXSLOT,PARTYSLOT, e.g.Bs0,1,0
						details = message.substring(2).split(",");
					...
					try {
						...
						case 'b':
							...
							if(m_players.containsKey(message.substring(2))) {
								...
								if(m.connect(GameServer.getDatabaseHost(), 
									...
									m.query("INSERT INTO kn_bans (ip) VALUE ('" + 
											o.getIpAddress()
											+ "')");
									m.close();
								}
						...
						case 'B':
							...
							if(m.connect(GameServer.getDatabaseHost(), 
									...
									GameServer.getDatabasePassword())) {
								m.selectDatabase(GameServer.getDatabaseName());
								m.query("DELETE FROM kn_bans WHERE ip='" + 
										message.substring(2)
										+ "'");
								m.close();
							}
	 ...
	 * @param m
	 */
	public static void writeMessage(IoSession session, K3RNELMessage m) {
