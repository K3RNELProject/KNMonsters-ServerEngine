...
import net.k3rnel.server.backend.map.ServerMap.PvPType;
...
import net.k3rnel.server.battle.impl.PvPBattleField;
...
import net.k3rnel.server.network.message.ItemMessage;
import net.k3rnel.server.network.message.SpriteChangeMessage;
import net.k3rnel.server.network.message.shop.ShopBuyMessage;
import net.k3rnel.server.network.message.shop.ShopNoItemMessage;
import net.k3rnel.server.network.message.shop.ShopNoMoneyMessage;
import net.k3rnel.server.network.message.shop.ShopNoSpaceMessage;
import net.k3rnel.server.network.message.shop.ShopSellMessage;
...
public class PlayerChar extends Char implements Battleable, Tradeable {
	 ...
	 * Stored as following:
	 * 0 - 7   First Set of Badges
	 * 8 - 15  Second Set of Badges
	 * 16 - 23 Third Set of Badges
	 * 24 - 31 Fourth Set of Badges
	 * 32 - 35 Fifth Set of Badges
	 * 36 - 41 //TODO: Replace bytes with int. 
	 */
	...
	/**
	 * Sets how many steps this Player can repel monsters for
	 * @param steps
	 */
	...
	/**
	 * Returns how many steps this player can repel Monsters for
	 * @return
	 */
	public int getRepel() {
		return m_repel;
	...
	/**
	 * Releases a monster from box
	 * @param box
	 * @param slot
	 */
	public void releaseMonster(int box, int slot) {
		/* If the box doesn't exist, return */
		if(m_boxes[box] == null)
			return;
		/* Check if the monster exists */
		if(m_boxes[box].getMonster(slot) != null) {
			if(m_boxes[box].getMonster(slot).getDatabaseID() > -1) {
				/* This box exists and the monster exists in the database */
				int id = m_boxes[box].getMonster(slot).getDatabaseID();
				...
				if(m.connect(GameServer.getDatabaseHost(), 
					...
					m.query("DELETE FROM kn_monsters WHERE id='" + id + "'");
					m.close();
					m_boxes[box].setMonster(slot, null);
				}
			} else {
				/*
				 * This Monster or box has not been saved to the
	...
	/**
	 * Swaps monsters between box and party 
	 * @param box
	 * @param boxSlot
	 * @param partySlot
	 */
	public void swapFromBox(int box, int boxSlot, int partySlot) {
		...
		if(m_boxes[box] == null) {
			m_boxes[box] = new MonsterBox();
			m_boxes[box].setMonsters(new Monster[30]);
		}
		/* Make sure we're not depositing our only Monster */
		if(getPartyCount() == 1) {
			if(m_monsters[partySlot] != null && m_boxes[box].getMonster(boxSlot) == null)
				return;
		}
		/* Everything is okay, let's get swapping! */
		Monster temp = m_monsters[partySlot];
		m_monsters[partySlot] = m_boxes[box].getMonster(boxSlot);
		m_boxes[box].setMonster(boxSlot, temp);
		if(m_monsters[partySlot] != null) {
			...
			if(this.getMap().getPvPType() == PvPType.ENFORCED) {
				...
				if(otherPlayer != null && this.getMap() == otherPlayer.getMap()) {
					if(otherPlayer.getX() >= this.getX() - 96 || 
						...
						ensureHealthyMonsters();
						otherPlayer.ensureHealthyMonsters();
	...
	/**
	 * Heals the player's monsters
	 */
	public void healMonsters() {
		for (Monster monster : getParty()) {
			if (monster != null) {
				monster.calculateStats(true);
				monster.reinitialise();
				monster.setIsFainted(false);
				for(int i = 0; i < monster.getMoves().length; i++) {
					if(monster.getMoves()[i] != null) {
						MonsterMove move = monster.getMoves()[i].getMove();
						monster.setPp(i, move.getPp() * (5 + monster.getPpUpCount(i)) / 5);
						monster.setMaxPP(i, move.getPp() * (5 + monster.getPpUpCount(i)) / 5);
					}
				}
			}
		}
	...
	public void removeTempStatusEffects() {
		for(Monster monster: getParty()) {
			if(monster != null) {
				monster.removeStatusEffects(false);
			}
		}
	}
	...
	/**
	 * If the player's first Monster in party has 0 HP, 
	 * it puts the first Monster in their party with more
	 * than 0 HP at the front
	 */
	public void ensureHealthyMonsters() {
		if(m_monsters[0] == null || m_monsters[0].getHealth() == 0) {
			for(int i = 1; i < 6; i++) {
				if(m_monsters[i] != null && m_monsters[i].getHealth() > 0) {
					swapMonsters(0, i);
					return;
				}
			}
	...
	/**
	 * Swaps two Monsters in a player's party
	 * @param a
	 * @param b
	 */
	public void swapMonsters(int a, int b) {
		if(a >= 0 && a < 6 && b >= 0 && b < 6) {
			Monster temp = m_monsters[a];
			m_monsters[a] = m_monsters[b];
			m_monsters[b] = temp;
	...
	/**
	 * Returns the amount of Monsters in this player's party
	 * @return
	 */
	...
	/**
	 * Returns the highest level monster in the player's party
	 * @return
	 */
	public int getHighestLevel() {
		int h = 0;
		for(int i = 0; i < m_monsters.length; i++) {
			if(m_monsters[i] != null && h < m_monsters[i].getLevel())
				h = m_monsters[i].getLevel();
		}
		return h;
	}

	/**
	 * Returns the Monster party of this player
	 */
	...
	/**
	 * Set the monster party of this player
	 */
	...
	public void fish(int rod) {
		if(this.lastFishingTime + 1000 < System.currentTimeMillis()) {
			if(this.getMap().caughtFish(this, this.getFacing(), rod)) {
				Monster mon = this.getMap().getWildMonster(this);
				//If we have both the required level to fish this thing up and the rod to do it
				if(this.getFishingLevel() >= DataService.getFishDatabase().getFish(mon.getSpeciesName()).getReqLevel() && rod >= DataService.getFishDatabase().getFish(mon.getSpeciesName()).getReqRod()) {
					this.addFishingExp(DataService.getFishDatabase().getFish(mon.getSpeciesName()).getExperience());
					this.ensureHealthyMonsters();
					m_battleField = new WildBattleField(DataService.getBattleMechanics(),this,mon);
				}
				...
				if(this.getMap() != null) {
					...
					if(m_repel <= 0 && this.getMap().isWildBattle(m_x, m_y, this)) {
						...
						m_battleField = new WildBattleField(
								...
								this,
								this.getMap().getWildMonster(this));
					...
					if(this.getX() % 32 == 0 || (this.getY() + 8) % 32 == 0) {
						for(int i = 0; i < m_monsters.length; i++) {
							/* 
							 * Monster only have their happiness 
							 * increased by walking if it is below 70
							 */
							if(m_monsters[i] != null && m_monsters[i].getHappiness() < 70)
								m_monsters[i].setHappiness(m_monsters[i].getHappiness() + 1);
						}
					}
				}
				return true;
	 ...
	 * @param boxes
	 */
	public void setBoxes(MonsterBox [] boxes) {
		m_boxes = boxes;
	}
	 ...
	 * @return
	 */
	public MonsterBox[] getBoxes() {
		return m_boxes;
	}

	/**
	 * Stores a caught Monster in the player's party or box
	 * @param p
	 */
	public void catchMonster(Monster mon) {
		Date d = new Date();
		String date = new SimpleDateFormat ("yyyy-MM-dd:HH-mm-ss").format (d);
		mon.setDateCaught(date);
		mon.setOriginalTrainer(this.getName());
		mon.setDatabaseID(-1);
		addMonster(mon);
		addTrainingExp(1000/mon.getRareness());
	}

	/**
	 * Adds a monster to this player's party or box
	 * @param p
	 */
	public void addMonster(Monster mon) {
		...
		for(int i = 0; i < m_boxes.length; i++) {
			if(m_boxes[i] != null) {
				...
				for(int j = 0; j < m_boxes[i].getMonsters().length; j++) {
					if(m_boxes[i].getMonster(j) == null) {
						m_boxes[i].setMonster(j, mon) ;
						return;
					}
				}
			} else {
				/* We need a new box */
				m_boxes[i] = new MonsterBox();
				m_boxes[i].setMonsters(new Monster[30]);
				m_boxes[i].setMonster(0, mon);
				break;
			}
		}
	}
	...
	public void sendBoxInfo(int j) {
		...
		if(m_boxes[j] == null) {
			m_boxes[j] = new MonsterBox();
			m_boxes[j].setMonsters(new Monster[30]);
			m_tcpSession.write("B");
		}
		/* Else send all mons in box */
		String packet = "";
		for(int i = 0; i < m_boxes[j].getMonsters().length; i++) {
			if(m_boxes[j].getMonster(i) != null)
				packet = packet + m_boxes[j].getMonster(i).getSpeciesNumber() + ",";
			else
				packet = packet + ",";
		}
	...
	/**
	 * Returns the index of the monster in the player's party
	 * @param p
	 * @return
	 */
	public int getMonsterIndex(Monster m) {
		for(int i = 0; i < m_monsters.length; i++) {
			if(m_monsters[i] != null) {
				if(m.compareTo(m_monsters[i]) == 0)
					return i;
			}
		}
		return -1;
	...
	public void updateClientParty(int i) {
		if(this.getParty()[i] != null) {
			m_tcpSession.write("Pi" + i + MonsterSpecies.getDefaultData().getMonsterByName(this.getParty()[i].getSpeciesName()).getSpeciesNumber() + "," +
	...
	/**
	 * Updates stats for a Monster
	 * @param i
	 */
	public void updateClientMonsterStats(int i) {
		if(m_monsters[i] != null) {
			m_tcpSession.write("PS" + i + m_monsters[i].getHealth() + "," +
					m_monsters[i].getStat(0) + "," +
					m_monsters[i].getStat(1) + "," +
					m_monsters[i].getStat(2) + "," +
					m_monsters[i].getStat(3) + "," +
					m_monsters[i].getStat(4) + "," +
					m_monsters[i].getStat(5));
