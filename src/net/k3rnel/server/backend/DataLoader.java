		...
		try {
			...
			while(reader.hasNextLine()) {
				...
				if(line.equalsIgnoreCase("[npc]")) {
					...
					//Load possible Monsters
					line = reader.nextLine();
					if(!line.equalsIgnoreCase("NULL")) {
						details = line.split(",");
						HashMap<String, Integer> enemies = new HashMap<String, Integer>();
						for(int i = 0; i < details.length; i = i + 2) {
							enemies.put(details[i], Integer.parseInt(details[i + 1]));
						}
						npc.setPossibleMonsters(enemies);
					}
					...
					t.setRequestedMonster(reader.nextLine(), 
