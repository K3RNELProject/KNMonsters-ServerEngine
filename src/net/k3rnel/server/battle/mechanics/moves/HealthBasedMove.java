 ...
 * @author Ben
 */
public class HealthBasedMove extends MonsterMove {

    private boolean m_highHp;
    
    public HealthBasedMove(MonsterType type, double accuracy, int pp, boolean highHp) {
    ...
    public int use(BattleMechanics mech, Monster user, Monster target) {
        double healthPercent = ((double)user.getHealth() / (double)user.getStat(Monster.S_HP) * 100.0);  
