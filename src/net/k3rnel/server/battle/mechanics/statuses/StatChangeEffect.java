...
public class StatChangeEffect extends StatusEffect {
    ...
    private String getDefaultDescription() {
        ...
        return "'s " + Monster.getStatName(m_stat) + " was " + adv + adj + ".";
    }
    ...
    /**
     * Called when a monster with this status effect switches out.
    ...
    private StatMultiplier getMultiplier(Monster p) {
        StatMultiplier mul = null;
        if (m_stat == Monster.S_ACCURACY) {
            mul = p.getAccuracy();
        } else if (m_stat == Monster.S_EVASION) {
            mul = p.getEvasion();
        } else {
    ...
    /**
     * Applies the initial effects of the status to a monster but does not add
     * the status to the list of statuses the monster has.
     */
    public boolean apply(Monster p) {
        
        // Abilities do not prevent monster from lowering their own stats.
        if (getInducer() != p) {        
            ...
            if (!m_raise
                    && (m_stat == Monster.S_ATTACK)
                    && p.hasAbility("Hyper Cutter")) {
                return false;
            }

            if (!m_raise
                    && (m_stat == Monster.S_ACCURACY)
        ...
        if (total == 0) {
            ...
            p.getField().showMessage(p.getName()
                + "'s " + Monster.getStatName(m_stat) + " won't go " + keyword + "!");
            return false;
        }
        return true;
    }
