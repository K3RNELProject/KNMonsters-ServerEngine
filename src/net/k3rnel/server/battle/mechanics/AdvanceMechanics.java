...
/**
 * This class represents the mechanics in the advanced generation of monster.
 * @author Colin
 */
public class AdvanceMechanics extends BattleMechanics {
    ...
    public int calculateStat(Monster p, int i) throws StatException {
     ...
     */
    public boolean attemptHit(MonsterMove move, Monster user, Monster target) {
    ...
    public boolean isCriticalHit(MonsterMove move, Monster user, Monster target) {
     ...
     */
    public boolean isMoveSpecial(MonsterMove move) {
        return move.getType().isSpecial();
    }
    
    public strictfp int calculateDamage(
            MonsterMove move,
            Monster attacker, Monster defender, boolean silent) {
        
        final BattleField field = attacker.getField();
        MonsterType moveType = move.getType();
     ...
     * points per stat. The monster's level must be in the interval [1, 100].
     */
    public void validateHiddenStats(Monster p) throws ValidationException {
        ...
        if (evs > 510) {
            throw new ValidationException(
                    "A monster cannot have more than 510 EVs in total.");
        }
    }
