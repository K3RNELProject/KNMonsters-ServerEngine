...
import net.k3rnel.server.battle.mechanics.moves.MoveList.OneHitKillMove;
    ...
    static {
        ...
        new EffectClause("Sleep Clause", SleepEffect.class) {
            public String getClauseDescription() {
                return "Only one monster on each team may be afflicted "
        ...
        new Clause("Item Clause") {
            public String getClauseDescription() {
                return "No two monsters on a team may begin the battle "
                        + "holding the same item.";
            }
            public boolean isTeamValid(BattleField field, Monster[] team, int idx) {
        ...
        new Clause("Species Clause") {
            public String getClauseDescription() {
                return "No two monsters on a team may have the same species.";
            }
            public boolean isTeamValid(BattleField field, Monster[] team, int idx) {
                Set<String> set = new HashSet<String>();
                for (int i = 0; i < team.length; ++i) {
        ...
        new Clause("Random Battle") {
            ...
            public boolean isTeamValid(BattleField field, Monster[] team, int idx) {
                for (int i = 0; i < team.length; ++i) {
                    ...
                    team[i] = Monster.getRandomMonster(
        ...
        new Clause("Level Balance") {
            public String getClauseDescription() {
                return "Multiplies 0.074 by a monster's base stat total "
                        + "and subtracts the result from 113; the monster's "
                        + "level is set to the integer nearest this value. "
                        + "This gives each monster a level in the closed "
            ...
            public boolean isTeamValid(BattleField field, Monster[] team, int idx) {
                for (int i = 0; i < team.length; ++i) {
                    Monster mon = team[i];
                    if (mon != null) {
                        mon.setLevel(mon.getBalancedLevel());
                        mon.calculateStats(true);
                    }
                }
                return true;
            }
                ...
                if (move instanceof StatusMove) {
                    ...
                    for (int i = 0; i < effects.length; ++i) {
                        ...
                        if (eff instanceof StatChangeEffect) {
                            ...
                            if ((stat.getStat() == Monster.S_EVASION)
                                    && stat.isRaise()) {
                                failure = true;
                                break;
                            }
    ...
    public static class PendanticDamageClause extends Clause {
        ...
        public String getClauseDescription() {
            return "By default, Shoddy Battle does not limit the display of "
                    + "damage done to a money to its remaining health. "
