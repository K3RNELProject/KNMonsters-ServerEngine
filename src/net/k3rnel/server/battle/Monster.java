/*
 * Monster.java Created on December 13, 2006, 5:38 PM This file is a part of
...
import net.k3rnel.server.battle.mechanics.clauses.Clause.PendanticDamageClause;
...
import net.k3rnel.server.battle.mechanics.statuses.AwesomeEffect;
...
/**
 * This class represents a monster in a battle. Its stats are automatically
...
public class Monster extends MonsterSpecies {
	...
	/* Stores the EXP growth rate of the Monster */
	@Element
	transient private ExpTypes                m_expType;
	@Element
	private int                               m_happiness;
	/* Stores the evolution this Monster is waiting to evolve to */
	private MonsterEvolution                     m_evolution      = null;

	// Intrinsic statistics.
	@Element
	private int                               m_level          = -1;
	@Element
	private MonsterNature                     m_nature;
	...
	/**
	 * Returns this monster's contest stats in string format NOTE: Only used for
	 * saving MySQL
	 * 
	 * @return
	 */
	...
	/**
	 * Sets the contest stat of the monster. NOTE: Use S_BEAUTY, S_CUTE, etc. for
	 * i
	 * 
	 * @param i
	 * @param amount
	...
	/**
	 * Returns the contest of the monster. NOTE: Use S_BEAUTY, S_CUTE, etc.
	 * 
	 * @param i
	 */
	...
	/**
	 * Returns true if this Monster is weak against Monster b
	 * 
	 * @param b
	 * @return
	 */
	public boolean hasTypeWeakness(Monster b) {
	...
	/**
	 * Returns true if this Monster is waiting to evolve
	 * 
	 * @return
	 */
	...
	/**
	 * Sets if this Monster is waiting to evolve and the evolution it is waiting
	 * to go to
	 * 
	 * @param e
	 */
	public void setEvolution(MonsterEvolution e) {
		m_evolution = e;
	}

	/**
	 * Returns true if a monster knows the move
	 * 
	 * @param move
	 * @return
	 */
	 ...
	 * @param p
	 *          - The player that owns the Monster
	 */
	public void evolutionResponse(boolean allow, PlayerChar p) {
		if (m_evolution != null) {
			/* Get the index of the Monster in the player's party */
			int index = p.getMonsterIndex(this);

			if (allow) {
				/* The player is allowing evolution, evolve the Monster */
				this.evolve(MonsterSpecies.getDefaultData().
						getMonsterByName(m_evolution.getEvolveTo()));
			}
			/* Retrieve the Monster data */
			MonsterSpecies monData = MonsterSpecies.getDefaultData().
			getMonsterByName(getSpeciesName());
			...
			String move = "";
			/* Generate a list of moves this Monster wants to learn */
			m_movesLearning.clear();
			for (int i = oldLevel + 1; i <= level; i++) {
				if (monData.getLevelMoves().get(i) != null) {
					move = monData.getLevelMoves().get(i);
					if (move != null && !move.equalsIgnoreCase("") && !hasMove(move))
						m_movesLearning.add(move);
				}
			}
			/* Save the Monster's level */
			setLevel(level);
			/* Update the client with new Monster information */
			p.updateClientParty(index);
			/* Inform the client this Monster wants to learn new moves */
			for (int i = 0; i < m_movesLearning.size(); i++) {
				p.getTcpSession().write("Pm" + index + m_movesLearning.get(i));
			}
			p.updateClientMonsterStats(index);
		}
	}

	/**
	 * Sets if this monster is fainted
	 * 
	 * @param b
	 */
	...
	/**
	 * Create a substitute to take hits for this monster.
	 */
	...
	/**
	 * Return whether this monster has a substitute.
	 */
	...
	public Monster(BattleMechanics mech, MonsterSpecies species,
			MonsterNature nature, String ability, String item, int gender, int level,
	...
	/**
	 * Create and validate a new monster.
	 */
	public Monster(BattleMechanics mech, MonsterSpecies species,
			MonsterNature nature, String ability, String item, int gender, int level,
	...
	/**
	 * Get a random Monster object.
	 */
	public static Monster getRandomMonster(ModData data, BattleMechanics mech) {
		...
		MonsterNature nature = MonsterNature.getNature(random.nextInt(25));
		MonsterSpeciesData speciesData = data.getSpeciesData();
		MonsterSpecies species = new MonsterSpecies(speciesData, random
		...
		Monster mon = new Monster(mech, species, nature, ability, item, gender, 100,
	...
	/**
	 * Returns a random Monster based on a species name and a level
	 * 
	 * @param speciesName
	 * @param level
	 * @return
	 */
	public static Monster getRandomMonster(String species, int level) {
		Monster mon;
		Random random = DataService.getBattleMechanics().getRandom();
		/*
		 * First obtain species data
		 */
		MonsterSpecies ps = MonsterSpecies.getDefaultData().getMonsterByName(species);
		MoveListEntry[] moves = new MoveListEntry[4];
		/*
		 * Generate a list of possible moves this Monster could have at this level
		 */
		 ...
		 */
		String[] abilities = MonsterSpecies.getDefaultData().getMonsterByName(
				species).getAbilities();
		/* First select an ability randomly */
		String ab = abilities[random.nextInt(abilities.length)];
		/*
		 * Now lets create the monster itself
		 */
		mon = new Monster(DataService.getBattleMechanics(), ps, MonsterNature
				.getNature(random.nextInt(MonsterNature.getNatureNames().length)), ab,
				null, Monster.generateGender(ps.getPossibleGenders()), level, new int[] {
		...
		mon.setBaseExp(ps.getBaseEXP());
		mon.setExpType(ps.getGrowthRate());
		mon.setExp(DataService.getBattleMechanics().getExpForLevel(mon, level));
		mon.setHappiness(ps.getHappiness());
		mon.setRareness(ps.getRareness());
		return mon;
	}
	 ...
	 */
	public static ModData loadTeam(File f, Monster[] team) {
		...
		try {
			...
			synchronized (MonsterSpecies.class) {
				MonsterSpeciesData data = MonsterSpecies.getDefaultData();
				MonsterSpecies.setDefaultData(modData.getSpeciesData());
				try {
					monster = (Monster[]) obj.readObject();
				} finally {
					MonsterSpecies.setDefaultData(data);
				}
			}
			if (monster != null) {
				System.arraycopy(monster, 0, team, 0, team.length);
			}
			obj.close();
		} catch (IOException e) {
	...
	/**
	 * Set whether this monster is shiny.
	 */
	...
	/**
	 * Return whether this monster is shiny.
	 */
	...
	/**
	 * Return the name of this monster's ability.
	 */
	...
	/**
	 * Set this monster's ability. If ignoreTransferability is true then the
	...
	/**
	 * Return the name of this monster's item.
	 */
	...
	public void validate(ModData data) throws ValidationException {
		...
		for (int i = 0; i < m_move.length; ++i) {
			...
			if (move != null) {
				...
				"This monster learns two of the same move."); }
				set.add(name);
				if (!canLearn(speciesData, name)) { throw new ValidationException(
						"This monster cannot learn " + name + "."); }
		...
		if (moveCount == 0) {
			// Monster must have at least one move.
			throw new ValidationException("This monster learns no moves.");
		} else if (moveCount > 4) { throw new ValidationException(
		"This monster learns move than four moves."); }

		int genders = getPossibleGenders();
		if (((genders & m_gender) == 0) && ((genders != 0) || (m_gender != 0))) { throw new ValidationException(
		"This monster has an invalid gender."); }
		...
		if ((m_itemName != null)
				...
				"This monster's item is invalid."); }
	}
	...
	/**
	 * Recalculates this Monster's stats. If reset is true, the Monster is also
	 * healed fully
	 * 
	 * @param reset
	 */
	...
	/**
	 * Calculate this monster's stats.
	 */
	private void initialise() throws StatException {
		...
		for (int i = 0; i < m_move.length; ++i) {
			if (m_move[i] != null) {
				...
				MonsterMove move = m_move[i].getMove();
				if (move != null) {
	...
	/**
	 * Get this monster's teammates, including this monster.
	 */
	public Monster[] getTeammates() {
		if (m_field == null) { return null; }
		return m_field.getParty(m_party);
	}

	/**
	 * Get the name of this monster's trainer.
	 */
	...
	/**
	 * Get the Monster that this Monster is fighting in a battle.
	 */
	public Monster getOpponent() {
		if (m_field == null) { return null; }
		Monster[] active = m_field.getActiveMonster();
		return active[(m_party == 0) ? 1 : 0];
	}

	/**
	 * Return whether a monster is a particular type.
	 */
	...
	/**
	 * Get the (additive) critical hit ability of this Monster.
	 */
	...
	/**
	 * Return whether this Monster is immune to critical hits.
	 */
	...
	/**
	 * Get one of this monster's moves.
	 */
	...
	/**
	 * This method is called when the monster is just about to execute its turn.
	 * 
	 * @param turn
	...
	/**
	 * Return whether it is the monster's first turn out.
	 */
	...
	/**
	 * Switch in this monster.
	 */
	public void switchIn() {
		// No iterator - it will freak out if switchIn() adds new statuses.
		m_lastMove = null;
		m_firstTurn = true;
		// Inform MonsterMoves that their potential user is switching in.
		for (int i = 0; i < m_move.length; ++i) {
	...
	/**
	 * Return the original ability of this monster.
	 */
	...
	/**
	 * Return the effect that vetoes the use of a particular one of this monster's
	 * moves.
	 */
	...
	/**
	 * Return whether this monster has a particular effect.
	 */
	...
	/**
	 * Return whether this Monster has a particular class of effect.
	 */
	...
	/**
	 * Return the effect applied to this monster of a particular lock or null if
	...
	/**
	 * Return the effect of a particular class applied to this monster, or null if
	...
	/**
	 * Return whether this monster has a particular ability.
	 */
	...
	/**
	 * Return whether this monster has a particular item.
	 */
	...
	/**
	 * Return whether this monster is active (able to choose moves and switch).
	 */
	...
	/**
	 * Get the display name of this monster (i.e. its nickname).
	 */
	@Override
	...
	/**
	 * Return whether this monster can switch.
	 */
	...
	/**
	 * Remove a status effect from this monster.
	 */
	...
	/**
	 * Remove a class of statuses from this monster.
	 */
	...
	/**
	 * Attach this monster to a battle field.
	 */
	...
	/**
	 * Detaches a battlefield from the Monster
	 */
	...
	/**
	 * Get this monster's party. This will be in the range [0, <b>parties</b> - 1]
	...
	/**
	 * Get this monster's position on the field to which it is attached.
	 */
	public int getId() {
		return m_id;
	}
	...
	/**
	 * Get the field to which this monster is attached.
	 */
	...
	/**
	 * Get the name of this monster's moves.
	 */
	...
	/**
	 * Determine whether this monster has fainted.
	 */
	...
	/**
	 * Get the effectiveness of this monster attacking a particular type.
	 */
	public static double getEffectiveness(List<?> statuses, MonsterType move,
			MonsterType monster, boolean enemy) {
		double expected = move.getMultiplier(monster);
		synchronized (statuses) {
			Iterator<?> i = statuses.iterator();
			while (i.hasNext()) {
				StatusEffect eff = (StatusEffect) i.next();
				if (eff.isActive() && eff.isEffectivenessTransformer(enemy)) {
					double actual = eff.getEffectiveness(move, monster, enemy);
	...
	public double getEffectiveness(MonsterType move, MonsterType monster,
			boolean enemy) {
		return getEffectiveness(m_statuses, move, monster, enemy);
	}

	/**
	 * Is this monster immobilised?
	 * 
	 * @param exception
	...
	/**
	 * Transform a move based on the status effects applied to the monster.
	 * 
	 * @param enemy
	 *          whether this Monster is an enemy
	 */
	...
	/**
	 * Get the last move used by this monster, or null if the monster has not used
	...
	/**
	 * Use one of this monster's intrinsic moves.
	 */
	public int useMove(int i, Monster target) {
		if (i == -1) {
	 ...
	 */
	public int useMove(MoveListEntry move, Monster target) {
		MonsterMove pmove = move.getMove();
	 ...
	 */
	public int useMove(MonsterMove move, Monster target) {
		if (!move.attemptHit(m_mech, this, target)) { return 0; }
		return move.use(m_mech, this, target);
	}

	/**
	 * Inform that this monster was damaged.
	 */
	private void informDamaged(Monster source, MoveListEntry entry, int damage) {
		int size = m_statuses.size();
		for (int i = 0; i < size; ++i) {
	...
	/**
	 * Change the health of this monster, doing damage to a substitute if one is
	 * present.
	 */
	...
	/**
	 * Change the health of this monster, optionally hitting through a substitute.
	 */
	public void changeHealth(int hp, boolean throughSubstitute) {
		...
		if (!hasSubstitute() || throughSubstitute || (hp > 0)) {
			...
			if (m_field != null) {
				m_field.informMonsterHealthChanged(this, display);
	...
	/**
	 * Cause this monster to faint.
	 */
	public void faint() {
		m_hp = 0;
		m_fainted = true;
		if (m_field != null) {
			m_field.informMonsterFainted(m_party, getId());
			m_field.checkBattleEnd(m_party);
		}
	}

	/**
	 * Get the health of this monster.
	 */
	...
	/**
	 * Inform listeners that a status effect was applied this monster.
	 */
	private void informStatusListeners(Monster source, StatusEffect eff,
			boolean applied) {
		synchronized (m_statuses) {
			int size = m_statuses.size();
			for (int i = 0; i < size; ++i) {
	...
	/**
	 * Return whether this monster must struggle if it wants to use a move.
	 */
	...
	/**
	 * Check whether the effects present on this monster permit the application of
	 * the given status effect to this monster.
	 */
	public boolean allowsStatus(StatusEffect eff, Monster source) {
		Iterator<StatusEffect> i = m_statuses.iterator();
		while (i.hasNext()) {
	...
	/**
	 * Add a status effect to this monster.
	 */
	public StatusEffect addStatus(Monster source, StatusEffect eff) {
	...
	/**
	 * Add a status effect to this monster caused by item. 
	 */
	...
	public MonsterNature getNature() {
		return m_nature;
	}
	...
	/**
	 * Returns 0 if they are the same Monster
	 * 
	 * @param monster
	 * @return
	 */
	public int compareTo(Monster monster) {
		if (this.getDateCaught() != null && monster.getDateCaught() != null
				&& monster.getDateCaught().equalsIgnoreCase(this.getDateCaught())) return 0;
		if (this.getDatabaseID() != -1 && monster.getDatabaseID() != -1
				&& monster.getDatabaseID() == this.getDatabaseID()) return 0;
		if (monster.getSpeciesName() == this.getSpeciesName()
				&& monster.getStat(0) == this.getStat(0) && monster.getStat(1) == this.getStat(1))
			return 0;
		return -1;
	}

	/**
	 * Reinitialises the Monster
	 * 
	 * @param b
	 */
	public void reinitialise() {
		...
		if(m_move[0] != null && !MonsterSpecies.getDefaultData().
				canLearn(MonsterSpecies.getDefaultData().
						getMonsterByName(this.getSpeciesName()), getMoveName(0))) {
			m_move[0] = null;
		}
		if(m_move[1] != null && !MonsterSpecies.getDefaultData().
				canLearn(MonsterSpecies.getDefaultData().
						getMonsterByName(this.getSpeciesName()), getMoveName(1))) {
			m_move[1] = null;
		}
		if(m_move[2] != null && !MonsterSpecies.getDefaultData().
				canLearn(MonsterSpecies.getDefaultData().
						getMonsterByName(this.getSpeciesName()), getMoveName(2))) {
			m_move[2] = null;
		}
		if(m_move[3] != null && !MonsterSpecies.getDefaultData().
				canLearn(MonsterSpecies.getDefaultData().
						getMonsterByName(this.getSpeciesName()), getMoveName(3))) {
			m_move[3] = null;
		}
	}
	...
	/**
	 * Evolves this monster into the new species
	 * 
	 * @param species
	 */
	private void evolve(MonsterSpecies species) {
		...
		try {
			String[] abilities = MonsterSpecies.getDefaultData()
	...
	/**
	 * Returns the EXP of the Monster
	 * 
	 * @return
	 */
	...
	/**
	 * Sets the base EXP of the Monster
	 * 
	 * @param baseEXP
	 */
	...
	/**
	 * Sets the happiness of the Monster
	 * 
	 * @param happiness
	 */
	...
	/**
	 * Returns the happiness of the Monster
	 * 
	 * @return
	 */
	...
	/**
	 * Sets the EXP growth rate of the Monster
	 * 
	 * @param growthRate
	 */
	...
	/**
	 * Returns the EXP growth rate of the Monster
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the base EXP of the Monster
	 * 
	 * @return
	 */
	...
	/**
	 * Get the date this monster was caught
	 */
	...
	/**
	 * Set the date this monster was caught
	 * 
	 * @param date
	 */
